...
function skullmask:OnTakeDamage(e, amount, flags, source, inv)
    ...
    if player.Velocity:Length() >= player.MoveSpeed * 3.5 then
...
if EID then
    EID:addTrinket(skullmask.id, "↑ +0.15 Speed#20% chance to avoid damage while moving near full speed#{{Blank}} (10% chance per Luck to roll with advantage)")
end
